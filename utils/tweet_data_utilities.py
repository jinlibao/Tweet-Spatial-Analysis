...
from utils import shapely_utilities as su
...
class TweetData:
    ...
    def create_dataframe(self,
        ...
        for row_idx in range(0, data_df.shape[0]):
            ...
            self.df.loc[row_idx, 'x'], self.df.loc[row_idx, 'y'] = au.lon_lat_to_east_north(
                self.df['lon'][row_idx],
                self.df['lat'][row_idx])
        ...
        for row in self.df.itertuples():
            ...
            for row2 in self.df.itertuples():
                if row2.id != selected_point_id:
                    if a > 0.0 and b > 0.0:
                        if au.is_point_inside_ellipse(xo, yo, a, b, theta, row2.x, row2.y):
    ...
    def create_dissolve(self, id_value, idx, grouped_idxs):
        ...
        if any(ellipses['x']):
            new_data = su.dissolve_ellipses(ellipses)
        else:
            new_data = dict()
            new_data['x'] = []
            new_data['y'] = []
    ...
    def calculate_ellipse_properties(self, row_id, idx, area, xy_ratio):
        ...
        if area == 0.0 or area == -1:
            ...
            return au.calculate_ellipse_a_and_b_from_area_and_ratio(area, xy_ratio_f)
...
class MapTweetData:
    ...
    def __init__(self, tweet_data, config):
        ...
        self.histogram_count = hu.HistogramData(self.tweet_data_df['count'], bins_text_list=config.bins_count_text, bins_list=config.bins_count)
        self.histogram_area = hu.HistogramData(self.tweet_data_df['area'], bins_list=100)
        self.histogram_distance = hu.HistogramData(self.tweet_data_df['distance'], bins_list=50)
        self.histogram_ratio = hu.HistogramData(self.tweet_data_df['ratio'], bins_text_list=config.bins_ratio_text, bins_list=config.bins_ratio)
        self.histogram_dissolve = hu.HistogramData(self.tweet_data_df['dissolve_area'], bins_list=100)
...
class TweetDataController:
    ...
    def __init__(self, pre_processor, config, user_info):
        ...
        self.histogram_controller_count = hu.HistogramController(self.active_dataset.histogram_count.df_histogram_cds, self.filter_circles_by_count)
        self.histogram_controller_area = hu.HistogramController(self.active_dataset.histogram_area.df_histogram_cds, self.filter_circles_by_area)
        self.histogram_controller_distance = hu.HistogramController(self.active_dataset.histogram_distance.df_histogram_cds, self.filter_circles_by_distance)
        self.histogram_controller_ratio = hu.HistogramController(self.active_dataset.histogram_ratio.df_histogram_cds, self.filter_circles_by_ratio)
        self.histogram_controller_dissolve = hu.HistogramController(self.active_dataset.histogram_dissolve.df_histogram_cds, self.filter_circles_by_dissolve)
