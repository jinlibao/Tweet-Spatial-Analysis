...
class MapWidgets:
    ...
    def __init__(self, tweet_data_controller, config):
        ...
        self.toggle_legend = Toggle(label="Toggle Legend", active=False, width=100)
    ...
    def button_reset_filters_callback(self):
        ...
        self.tweet_data_controller.filter_circles_by_count(self.config.count[0], self.config.count[1])
        self.range_slider_count.value = values_reset
        values_reset = [self.config.area[0], self.config.area[1]]
        self.tweet_data_controller.filter_circles_by_area(self.config.area[0], self.config.area[1])
        self.range_slider_area.value = values_reset
        values_reset = [self.config.distance[0], self.config.distance[1]]
        self.tweet_data_controller.filter_circles_by_distance(self.config.distance[0], self.config.distance[1])
        self.range_slider_distance.value = values_reset
        values_reset = [self.config.ratio[0], self.config.ratio[1]]
        self.tweet_data_controller.filter_circles_by_ratio(self.config.ratio[0], self.config.ratio[1])
        self.range_slider_ratio.value = values_reset
        values_reset = [self.config.dissolve[0], self.config.dissolve[1]]
        self.tweet_data_controller.filter_circles_by_dissolve(self.config.dissolve[0], self.config.dissolve[1])
