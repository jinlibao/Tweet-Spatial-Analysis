...
PARTITION=$MORAN_PARTITION
NODES=$MORAN_NODES
NTASKS_PER_NODE=$MORAN_NTASKS_PER_NODE
MEM=$MORAN_MEM
...
PARTITION=$MORAN_GPU_PARTITION
NODES=$MORAN_GPU_NODES
NTASKS_PER_NODE=$MORAN_GPU_NTASKS_PER_NODE
MEM=$MORAN_GPU_MEM
export NCPU=`echo $NODES \* $NTASKS_PER_NODE|bc`
#sbatch --partition=$PARTITION --nodes=$NODES --ntasks-per-node=$NTASKS_PER_NODE --mem=$MEM task.sh

PARTITION=$MORAN_HUGEMEM_PARTITION
NODES=$MORAN_HUGEMEM_NODES
NTASKS_PER_NODE=$MORAN_HUGEMEM_NTASKS_PER_NODE
MEM=$MORAN_HUGEMEM_MEM
export NCPU=`echo $NODES \* $NTASKS_PER_NODE|bc`
#sbatch --partition=$PARTITION --nodes=$NODES --ntasks-per-node=$NTASKS_PER_NODE --mem=$MEM task.sh
